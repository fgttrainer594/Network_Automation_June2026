...
from paloalto_api import (
    ...
    get_paloalto_service_names,
    paloalto_service_exists,
    build_paloalto_service_payload,
    create_paloalto_service_object,
    verify_paloalto_service_object,
    get_paloalto_service_groups,
    paloalto_service_group_exists,
    find_missing_service_members,
    build_paloalto_service_group_payload,
    create_paloalto_service_group,
    verify_paloalto_service_group
)

SERVICE_CSV_FILE = "service_objects.csv"
SERVICE_GROUP_CSV_FILE = "service_groups.csv"

SERVICE_REPORT_FILE = "automation_report_paloalto_services.csv"
SERVICE_GROUP_REPORT_FILE = "automation_report_paloalto_service_groups.csv"

LOG_FILE = "automation_paloalto_services.log"
VENDOR = "Palo Alto"
...
log_and_print("INFO", "Starting Palo Alto service automation workflow")
...
# ============================================================
# PHASE 1: LOAD EXISTING PALO ALTO SERVICES
# ============================================================

service_success, existing_services, service_message = get_paloalto_service_objects()
...
existing_service_names = get_paloalto_service_names(existing_services)
...
try:
    with open(SERVICE_CSV_FILE, mode="r", newline="") as file:
        ...
        for line_number, row in enumerate(reader, start=2):
            ...
            if paloalto_service_exists(service_name, existing_services):
                log_and_print(
                    "WARNING",
            ...
            payload = build_paloalto_service_payload(
                service_name,
                protocol,
                destination_port,
                description
            ...
            create_success, create_message = create_paloalto_service_object(
                service_name,
                payload
            )
            ...
            verify_success, service_obj, verify_message = verify_paloalto_service_object(
                service_name
            )
            ...
            if verify_success:
                ...
                add_service_result(
                    ...
                    "CREATED",
                    "SUCCESS",
                    "Service object created and verified successfully. Commit may still be required."
                )
...
# ============================================================
# PHASE 3: LOAD EXISTING PALO ALTO SERVICE GROUPS
# ============================================================

group_success, existing_groups, group_message = get_paloalto_service_groups()
...
for group_name, group_data in service_groups.items():
    ...
    if paloalto_service_group_exists(group_name, existing_groups):
        log_and_print(
            "WARNING",
    ...
    payload = build_paloalto_service_group_payload(
        group_name,
        members,
        description
    )

    create_success, create_message = create_paloalto_service_group(
        group_name,
        payload
    )
    ...
    verify_success, group_obj, verify_message = verify_paloalto_service_group(
        group_name
    )
    ...
    if verify_success:
        ...
        add_service_group_result(
            ...
            "CREATED",
            "SUCCESS",
            "Service group created and verified successfully. Commit may still be required."
        )
...
# Final summary
log_and_print("INFO", "")
log_and_print("INFO", "Final Palo Alto Service Automation Summary")
log_and_print("INFO", "-" * 60)
...
log_and_print("INFO", "-" * 60)
log_and_print("INFO", "Palo Alto service automation workflow completed")